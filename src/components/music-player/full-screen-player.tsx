...
export function FullScreenPlayer() {
  ...
  const handleArtistClick = (e: any, name: string) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black flex flex-col animate-in slide-in-from-bottom duration-500 overflow-hidden">
        ...
        <div className="px-6 py-4 flex flex-col items-center justify-center space-y-6 md:space-y-8 min-h-full">
          ...
          <div className="w-full max-w-sm px-2 space-y-4">
            <div className="flex items-center justify-between gap-4">
                ...
                <div className="whitespace-nowrap overflow-hidden">
                  <p className={cn("text-xs md:text-sm text-primary font-bold uppercase tracking-widest inline-block opacity-80", artistNames.length > 30 && "animate-marquee")}>
                    {currentTrack.artists.primary.map((a, i) => (
                      <span key={i} onPointerDown={handlePointerDown} onPointerUp={handleInteraction(() => handleArtistClick(null, a.name))} className="hover:underline cursor-pointer">
              ...
              <Button variant="ghost" size="icon" onPointerDown={handlePointerDown} onPointerUp={handleInteraction(() => toggleLike(currentTrack))} className="text-neutral-400"><Heart className={cn(liked && "fill-primary text-primary")} /></Button>
            </div>

            <div className="space-y-2">
              <Slider value={[progress]} max={duration || 100} step={0.1} onValueChange={v => seek(v[0])} />
      ...
      {/* Ergonomic Mobile Controls - Visual Short buttons with Large Hitboxes */}
      <div className="shrink-0 z-10 bg-gradient-to-t from-black via-black to-transparent w-full pb-8 pt-2">
        <div className="flex items-center justify-center gap-12 max-w-xs mx-auto relative h-16">
          <Button variant="ghost" size="icon" onPointerDown={handlePointerDown} onPointerUp={handleInteraction(prevTrack)} className="text-white h-14 w-14 absolute left-0"><SkipBack fill="currentColor" /></Button>
          <Button onClick={togglePlay} className="bg-primary text-black rounded-full h-16 w-16 shadow-[0_0_30px_rgba(255,0,0,0.4)] transition-transform active:scale-90">
            {isBuffering ? <Loader2 className="animate-spin" /> : isPlaying ? <Pause fill="currentColor" /> : <Play fill="currentColor" />}
          </Button>
          <Button variant="ghost" size="icon" onPointerDown={handlePointerDown} onPointerUp={handleInteraction(nextTrack)} className="text-white h-14 w-14 absolute right-0"><SkipForward fill="currentColor" /></Button>
        </div>
      </div>
    </div>
  );
